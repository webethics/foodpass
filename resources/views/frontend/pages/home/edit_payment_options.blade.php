@extends('frontend.creaters.landing.landing')
	@section('pageTitle','Edit Payment Options')
	@section('content')
							...
							<div class="col storeprofilecol">
							   <div class="profilerow pt-2">
								  <div class="profileinfo">
									 ...
									 <form class="editprofileform password_form" action = "{{ url('save-payment-options')}}" method="post" enctype="multipart/form-data">
										{{ csrf_field() }}
										<div class="profilehdr">
										   <div class="profilelefthdr dashboardpfl_hdr editprofile_section">
											  <div class="infohdr editprofileinfo">
												 <div class="row">
                                                 @foreach($user as $singlePayment)
                                                  <input type = "hidden" name="payment_id[]" id="payment_id" value= "{{ $singlePayment['id'] }}"  />
													<div class="col-md-6 form-group mb-5">
													   <label class="payment_options">{{ $singlePayment['payment_options']}}</label> 
													   <span class="merchant_id">Merchant Id:</span><input name="payment_options[]" id="payment_options" class="form-control delivery_range_store" type="text" value = "{{ $singlePayment['merchant_id'] }}">
                                                    </div>
                                                @endforeach
												 </div>
												 <div class="col-md-12 editbtn pb-3 mt-5 text-center">
