                           ...
                           <div class="row">
                              <div class="col-md-4 addcpn_image">
                                 ...
                                 <div class="uploadimage_blk">
									...
									@php
										$photo =  coupon_image($coupons->id);
									 @endphp
                                    <div class="circle">
										@if($photo)
											<img class="profile-pic" src="{{$photo}}" style = "display:block;">
										@endif
                                    </div>
                                    <div class="p-image">
                              ...
                              <div class="col-md-8 addcpm_form">
                                 <form class="forumform addcouponform" action = "{{ url('/updatecoupon/'. $coupons->id ) }}" method="post" enctype="multipart/form-data">
								 {{ csrf_field() }}
                                    <div class="row">
                                       <div class="col-md-6 form-group">
                                          <label>Product Name</label>
                                          <input type="text" name="product_name" class="form-control" placeholder="Enter Product Name" value="{{ $coupons->product_name }}" id="product_name">
                                       </div>
									   @php
									   $start  = date('d-m-Y H:i', $coupons->coupon_start_time); 
									   $expire = date('d-m-Y H:i', $coupons->coupon_end_time); 
									   @endphp
                                       <div class="col-md-6 form-group">
                                          <label>Expire time and date</label>
                                          <div class="row expirerow">
                                             <div class="col-md-6 expireinput">
                                                <input id="datepicker1" name="coupon_start_time" class="form-control" placeholder="From" value="{{$start}}" />
                                             </div>
                                             <div class="col-md-6 expireinput">
                                                <input id="datepicker2" name="coupon_end_time" class="form-control" placeholder="Until" value="{{$expire}}" /> 
                                             </div>
                                          </div>
                                       </div>
                                    </div>
                                    <div class="row">
                                       <div class="col-md-6 form-group deliverY-block">
                                          ...
                                          <input type="text" name="delivery_range" id="delivery_range" class="form-control" placeholder="Range" value="{{$coupons->delivery_range}}">
                                       </div>
                                       <div class="col-md-6 form-group">
                                       ...
                                       <div class="col-md-12 form-group">
                                          ...
                                          <div class="row" style="margin:0">
                                                ...
                                                <label class="current_price">Current Price</label>
                                                   <input type="text" name="coupon_price" id="coupon_price" class="form-control" placeholder="Current Price" value="{{$coupons->coupon_price}}">
                                                </div>
                                                <div class="col-md-6 form-group" style="padding-left:0;">
                                                   <label class="discount_code">Discount Code</label>
                                                   <input type="text" name="discount_code" id="discount_code" class="form-control" placeholder="Discount Code" value="{{$coupons->discount_code}}">
                                                </div>
                                             </div>
                                             <div class="cpntype_hdr">
                                                <label>Select Coupon type</label>
                                                <div class="cpntype_btns">
												<input type = "hidden" id="coupon_type" name="coupon_type" value = "{{$coupons->coupon_type}}" />
												
												<input type = "hidden" id="free_coupon_type" name="free_coupon_type" value = "{{$coupons->free_coupon_type}}"
												/>
												@if($coupons->coupon_type == "")
                                                   <span class = "coupon_type cursor" data-id="1">€</span>
                                                   <span class = "coupon_type cursor" data-id="2">%</span>
                                                @else
												   <span class = "coupon_type cursor <?php if($coupons->coupon_type == 1) 
													   echo "active"; ?>" data-id="1">€</span>
                                                   <span class = "coupon_type cursor <?php if($coupons->coupon_type == 2) 
													   echo "active"; ?>" data-id="2">%</span>
												@endif
												
												<span class = "coupon_type_free cursor <?php if($coupons->free_coupon_type == 1)echo "active"; ?>" data-id="3">Free</span>
											 ...
											 @endphp
											 <?php
											 if($coupons->coupon_type == "1" || $coupons->coupon_type == "2"){
												$amount_Class = "inline-block"; 
											 }else{
												$amount_Class = "none"; 
											 }
											 
											  if($coupons->free_coupon_type == "1" ){
												$product_Class = "inline-block"; 
											 }else{
												$product_Class = "none"; 
											 }
											?>
                                             <div class="cpntype_input" id="euro_precent" style="display:{{$amount_Class}};">
                                               <label class="coupon_discount_label">Hoeveel € korting wil je geven?</label>
                                                <div class="cpntype_formgroup">
                                                   <input type="text" name="discount_1" id="discount_1" class="form-control" placeholder="Discount" value="{{$coupons->discount_1}}" />
                                                   <input type="text"  name="new_price_1" id="new_price_1" class="form-control" placeholder="New Price" value="{{$coupons->new_price_1}}" readonly />
                                                </div>
                                             </div>
                                  <div class="cpntype_input" id="free_product" style="display:{{$product_Class}};">
                                  <label class="coupon_discount_label">Naam product (Bijv. Gratis Cola of Gratis bezorging)</label>
												<div class="cpntype_formgroup">
                                                   <input type="text" name="free_product" id="free_product_input" class="form-control" placeholder="Product Name" value="{{$coupons->free_product}}" />
                                                </div>
                                             </div>
                                          </div>
                                       </div>
                                    ...
                                    <div class="row">
                                       <div class="col-md-12 form-group">
                                          <label>Details</label>
										  <input type = "hidden" name="coupon_image" value = "{{$coupons->coupon_image}}" id="coupon_image"> 
                                          <textarea class="form-control" rows="3" id="details" name="details">{{$coupons->details}}</textarea>
                                       </div>
                                    </div>
                                    <div class="row">
	...
	<div class="modal-dialog modal-lg subs-modal profile_photo_modal">
      ...
      <script>
         ...
         $(document).ready(function(){
            var pickup_delivery = "<?php echo $coupons->coupon_pickup_delivery; ?>";
            if(pickup_delivery == 1){
               $('input#toggle-on').attr('checked', 'checked');
            } else if (pickup_delivery == 2) {
               $('input#toggle-off').attr('checked', 'checked');
            }
         });
